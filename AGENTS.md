...
- TODO: after route-declaration factories like `AppRoutes.router()` are fully adopted, re-evaluate whether `RouterProvider` + `Link` should stay public or be reduced in favor of direct-router links so the convenience layer no longer depends on `RouterNavigation`.
